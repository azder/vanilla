...
($ => {
    ...
    class List {
        ...
        static of(value) {
            ...
            m[VAL] = nil(value) ? [] : Array.of(value);
            return m;
        }

        static from(value) {
            let m = new List(value);
            m[VAL] = nil(value) ? [] : Array.from(value);
            return m;
        }

        remap(fn) {
            return concat(
                [], // stops the complaining about "can't convert undefined to object"
                ...this[VAL].map(
    ...
    return Object.assign($, {
        ...
        map: curry((fn, obj)=>obj.map(fn)),

        plus: curry((a, b)=>a + b),

        first: a => nil(a) ? void 0 : (Array.isArray(a) ? a[0] : a),
        last:  a => nil(a) ? void 0 : (Array.isArray(a) ? a[a.length - 1] : a),
        ...
        compile: curry(template => Function(
            'data',
            `return \`${
                template
                    .replace(/\$\{([\s]*?[^;\s]+?[\s]*?)\}/g, (_, match)=> `\$\{data.${match.trim()}\}`)
                    .replace(/(\$\{(?!data\.)[^}]+\})/g, '')
                }\``
        ))
